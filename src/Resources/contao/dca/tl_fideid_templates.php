	...
	(
		...
		(
			...
			(
				...
				'button_callback'     => array('tl_fideid_templates', 'toggleIcon')
			),
			'show' => array
			(
...
class tl_fideid_templates extends Backend
{
	...
	public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_fideid_templates::published', 'alexf'))
		{
			return '';
		}
	...
	public function toggleVisibility($intId, $blnPublished)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_fideid_templates::published', 'alexf'))
		{
			$this->log('Not enough permissions to show/hide record ID "'.$intId.'"', 'tl_fideid_templates toggleVisibility', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}
		
		$this->createInitialVersion('tl_fideid_templates', $intId);
		
		// Trigger the save_callback
		if (is_array($GLOBALS['TL_DCA']['tl_fideid_templates']['fields']['published']['save_callback']))
		{
			foreach ($GLOBALS['TL_DCA']['tl_fideid_templates']['fields']['published']['save_callback'] as $callback)
			{
		...
		// Update the database
		$this->Database->prepare("UPDATE tl_fideid_templates SET tstamp=". time() .", published='" . ($blnPublished ? '' : '1') . "' WHERE id=?")
		               ->execute($intId);
		$this->createNewVersion('tl_fideid_templates', $intId);
	}
}
